...
function CrisisMap() {
    ...
    const markers = useMemo(() => incidents.map((inc) => {
        ...
        return ( <
            ...
            <
            Pin background = { isCritical ? '#ff3366' : '#00f0ff' }
            glyphColor = { '#ffffff' }
    ...
    return ( <
        ...
        <
        APIProvider apiKey = { apiKey } > { /* 🚨 CRITICAL: Explicit dimensions on the container and mapId for AdvancedMarkers */ } <
...

        {
            selectedIncident && ( <
                ...
                <
                Badge variant = { selectedIncident.severity >= 4 ? 'danger' : 'accent' }
                ...
                Button className = "w-full py-2 text-[9px] font-black uppercase"
                onClick = {
        ...
        span className = "text-xs font-mono font-bold text-secondary" > { incidents.length }
        SIGNAL_NODES < /span> <
        /div> <
        /Card> <
        /div> <
